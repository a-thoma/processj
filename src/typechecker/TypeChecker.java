...
public class TypeChecker extends Visitor<Type> {
	...
	// Now switch on the operators
	    ...
	    // &=, |=, %=
            if (!((vType.isIntegralType() && eType.isIntegralType()) || (vType.isBooleanType() && eType.isBooleanType()))) {
                as.type = new ErrorType();
		//--Error.addError(as, "Both right and left-hand-side operands of operator '" + as.opString() + "' must be either of boolean or integral type.", 3009);
	    }
            break;
	}
	...
	Log.log(be.line + ": Visiting a Binary Expression");
		...
		be.type = new ErrorType();
		//!!Error.addError(be, "Operator '" + op + "' requires operands of numeric type.", 3010);
	    }
            break;
        }
		...
		// Any two numeric types can be compared.
                ...
                be.type = new ErrorType();
		//!!Error.addError(be, "Operator '" + op + "' requires operands of the same type.", 3012);
	    }
	    break;
        }
        ...
        case BinaryExpr.OROR: {
            if (lType.isBooleanType() && rType.isBooleanType())
                ...
                be.type = new ErrorType();
		//!!Error.addError(be, "Operator '" + op + "' requires operands of boolean type.", 3013);
	    }
            break;
        }
		...
		be.type = new ErrorType();
		//!!Error.addError(be, "Operator '" + op + "' requires both operands of either integral or boolean type.", 3014);
	    }
            break;
        }
		...
		be.type = new ErrorType();
                //!!Error.addError(be, "Operator '" + op + "' requires operands of numeric type or string/boolean, string/numeric, or string/string type.", 3015);
	    }
            break;
        }
        // << >> >>>:
		...
		be.type = new ErrorType();
		//!!Error.addError(be, "Operator '" + op + "' requires left operand of integral type.", 3016);
	...
	// Now create a ChannelEndType based on the sharing attibutes of the channel type.
	...
	// Channel has write end shared; if .write then create a shared chanenl end, otherwise create a non-shared one.
        else if (ct.shared() == ChannelType.SHARED_WRITE)
            ce.type = new ChannelEndType((ce.isWrite() && ct.shared() == ChannelType.SHARED_WRITE) ? ChannelEndType.SHARED
					 : ChannelType.NOT_SHARED, ct.baseType(), end);
        else {
	    // Techinically we should never be able to reach this code.
	    ce.type = new ErrorType();
            //!!Error.addError(ce, "Unknown sharing status for channel end expression.", 3020);
	}
	...
	// Only channel ends, timers, and channels [see above] can be read.
	    ...
	    // Error.addError(cr, "Channel or Timer type required in channel/timer read.", 3021);
	...
	// If there is an extended rendez-vous block and we are not reading from a timer, check it.
	...
	if (fs.barriers().size() > 0 || fs.isPar()) {
	    //!! Error.error(..."Process already enrolled on barriers (a non-par for loop cannot enroll on barriers)");
	}
	...
	// ...
	// id.id....id::f(...)
	// should be looked up directly in the appropriate symbol table and candidates should only be taken from there.
	...
	// This should be procedures of the right name and the right number of paremeters from:
	// - The top level symbol table (the one associated with the file passed to the compiler.
	// - The symbol tables of any files imported by the top level file, but NOT what they import.	
        while (st != null) {
		...
		// compare to all other candidates ptd2.
                for (int j = 0; j < candidateProcs.size(); j++) {
                    ...
                    for (int k = 0; k < ptd1Params.nchildren; k++) {
                        candidate = candidate &&
			    (resolve(((ParamDecl) ptd2Params.child(k)).type())).typeAssignmentCompatible(resolve(((ParamDecl) ptd1Params.child(k)).type()));
...
//			    sg.statements().visit(this);
    ...
    public Type visitTernary(Ternary te) { 
  	Log.log(te.line + ": Visiting a ternary expression");
        ...
        if (!eType.isBooleanType())
            Error.addError(te, "Non-boolean Expression (" + eType.typeName() + ") found as test in ternary expression.", 3070);

	// e ? t : f
	// Primitive?(Type(t)) & Primitive?(Type(f)) & (Type(t) :=T Type(f) || Type(f) :=T Type(t)) =>
	// Type(e ? t : f) = ceiling(Type(t), Type(f))
	    ...
	    te.type = null; // TODO
	    // if both are of primitive type they must be the same. if they are of record type or protocol type use the inheritance rules for those.
	} else 
	...
	Log.log(ts.line + ": visiting a timeout statement.");
        ...
        if (!dType.isIntegralType())
            Error.error(ts, "Invalid type (" + dType.typeName() + ") in timeout statement, integral type required.", false, 3049);
	...
	Log.log(up.line + ": Visiting a unary post expression");
        ...
        if (up.expr() instanceof NameExpr || up.expr() instanceof RecordAccess || up.expr() instanceof ArrayAccessExpr) {
            if (!eType.isIntegralType() && !eType.isDoubleType() && !eType.isFloatType())
                up.type = Error.addError(up,"Cannot apply operator '" + up.opString() + "' to something of type " + eType.typeName() + ".", 3051);
        } else
	...
	Log.log(up.line + ": Visiting a unary pre expression");
        ...
        case UnaryPreExpr.MINUS:
            if (!eType.isNumericType())
                up.type = Error.addError(up,"Cannot apply operator '" + up.opString() + "' to something of type " + eType.typeName() + ".",3052);
            break;
        case UnaryPreExpr.NOT:
            if (!eType.isBooleanType())
                up.type = Error.addError(up, "Cannot apply operator '!' to something of type " + eType.typeName() + ".", 3053);
            break;
        case UnaryPreExpr.COMP:
            if (!eType.isIntegralType())
                up.type = Error.addError(up, "Cannot apply operator '~' to something of type " + eType.typeName() + ".", 3054);
            break;
	    ...
	    // TODO: protocol access
            ...
            if (!eType.isNumericType() && up.type == null)
                up.type = Error.addError(up,"Cannot apply operator '" + up.opString() + "' to something of type " + eType.typeName() + ".", 3056);
            break;
        }
