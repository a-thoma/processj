...
public class TypeChecker extends Visitor<Type> {
    ...
    public Type resolve(Type t) {
        ...
        if (t.isNamedType()) {
            ...
            Log.log("  > Resolved " + ((NamedType) t).name().getname() + " -> " + tt + "\n---");
            return tt;
        } else {
    ...
    public Type visitAltCase(AltCase ac) {
        ...
        if (ac.precondition() != null) {
            ...
            if (!t.isBooleanType())
                Error.addError(ac.precondition(), "Boolean type expected in precondition of alt case, found: " + t,
                        3035); // test ok
        }
        ac.guard().visit(this);
        ac.stat().visit(this);
    ...
    public Type visitAssignment(Assignment as) {
        ...
        // as.right().type = eType; // TODO: wouldn't this have been set in the visit
        // call already?
        ...
        case Assignment.EQ: {
            if (!Type.assignmentCompatible(vType, eType))
                as.type = Error.addError(as, "Cannot assign value of type " + eType.typeName() + " to variable of type "
        ...
        case Assignment.MINUSEQ:
            if (!Type.assignmentCompatible(vType, eType))
                ...
                as.type = Error.addError(as,
                        "Right hand side operand of operator '" + as.opString() + "' must be of numeric type.", 3005); // test
                                                                                                                       // OK
            else if (!vType.isNumericType() && as.op() != Assignment.PLUSEQ)
                as.type = Error.addError(as,
                        "Left hand side operand of operator '" + as.opString() + "' must be of numeric type.", 3006);
            else if (as.op() == Assignment.PLUSEQ && !(vType.isNumericType() || vType.isStringType()))
                as.type = Error.addError(as,
                        "Left hand side operand of operator '" + as.opString() + "' must be of numeric or string type.",
                        3036);
            break;
        ...
        case Assignment.RRSHIFTEQ:
            if (!vType.isIntegralType())
                as.type = Error.addError(as,
                        "Left hand side operand of operator '" + as.opString() + "' must be of integral type.", 3007);
            if (!eType.isIntegralType())
                as.type = Error.addError(as,
                        "Right hand side operand of operator '" + as.opString() + "' must be of integral type.", 3008);
            break;
        ...
        case Assignment.XOREQ:
            if (!((vType.isIntegralType() && eType.isIntegralType())
                ...
                as.type = Error.addError(as, "Both right and left-hand-side operands of operator '" + as.opString()
                        + "' must be either of boolean or integral type.", 3009);
            break;
        }
        if (as.type == null)
            as.type = vType;
    ...
    public Type visitBinaryExpr(BinaryExpr be) {
        ...
        case BinaryExpr.XOR: {
            if ((lType.isBooleanType() && rType.isBooleanType())
                ...
                be.type = Error.addError(be,
                        "Operator '" + op + "' requires both operands of either integral or boolean type.", 3014);
            break;
        }
        ...
        case BinaryExpr.MOD: {
            if (lType.isNumericType() && rType.isNumericType()) {
                ...
                be.type = Error.addError(be, "Operator '" + op
                        + "' requires operands of numeric type or string/boolean, string/numeric, or string/string type.",
                        3015);
            break;
        }
        // << >> >>>:
        ...
        default:
            be.type = Error.addError(be, "Unknown operator '" + op + "'.", 3018);
        }
        Log.log(be.line + ": Binary Expression has type: " + be.type);
        return be.type;
    }
    ...
    public Type visitChannelEndExpr(ChannelEndExpr ce) {
        ...
        // if the expression is an arrayof channels then .read or .write creates an
        // array of channel ends!
    ...
    // ClaimStat
    // TODO: check that you are only claiming channel ends
    ...
    public Type visitDoStat(DoStat ds) {
        ...
        if (!eType.isBooleanType())
            Error.addError(ds, "Non boolean Expression found as test in do-statement.", 3024);
    ...
    public Type visitForStat(ForStat fs) {
        ...
        for (Expression e : fs.barriers()) {
            ...
            if (!t.isBarrierType())
                Error.addError(fs.barriers().child(i), "Barrier type expected, found '" + t + "'.", 3025);
            i++;
        }
    ...
    public Type visitIfStat(IfStat is) {
        ...
        if (!eType.isBooleanType())
            Error.addError(is, "Non-boolean expression found as test in if-statement.", 3027);
        if (is.thenpart() != null)
                ...
                for (Object pd : procs.entries.values().toArray()) {
                    ...
                    if (ptd.formalParams().size() == in.params().size()) {
                        ...
                        for (int i = 0; i < in.params().size(); i++) {
                            candidate = candidate && Type.assignmentCompatible(
                                    resolve(((ParamDecl) ptd.formalParams().child(i)).type()),
                                    in.params().child(i).type);
                        }
                        if (candidate) {
            ...
            for (int i = 0; i < candidateProcs.size(); i++) {
                ...
                for (int j = 0; j < candidateProcs.size(); j++) {
                    ...
                    for (int k = 0; k < ptd1Params.nchildren; k++) {
                        candidate = candidate && Type.assignmentCompatible(((ParamDecl) ptd2Params.child(k)).type(),
    ...
    // NewMobile
    // TODO: you can only 'new' stuff that is a mobile -- perhaps this check from
    // the NameChecker should be here!
    ...
    public Type visitPrimitiveLiteral(PrimitiveLiteral pl) {
        ...
        // pl.type = null; // new NullType(li); TODO: Perhaps we need a null type and a
        // null value too ??
        // else {
    ...
    public Type visitProtocolLiteral(ProtocolLiteral pl) {
        ...
        for (int i = 0; i < pc.body().size(); i++) {
            ...
            if (!Type.assignmentCompatible(vType, eType))
                Error.addError(pl, "Cannot assign value of type '" + eType + "' to protocol field '" + name.getname()
                        + "' of type '" + vType + "'.", 3034);
        }
    ...
    public Type visitRecordAccess(RecordAccess ra) {
        ...
        if (tType.isStringType() && ra.field().getname().equals("length")) {
            ...
            if (tType.isRecordType()) {
                ...
                /*
                 * switch statements cannot be nested on the same protocol!!! keep a hashtable
                 * of the protocols we have switched on - and their tags!
                 * 
                 * | protocol Name | -> ProtocolCase
                 * 
                 * we have <expr>.<field> that means a field <field> in the tag associaged with
                 * the type of <expr>!
                 */
    ...
    public Type visitSwitchStat(SwitchStat ss) {
        ...
        if ((!eType.isIntegralType() && !eType.isProtocolType()) || eType.isLongType())
            Error.addError(ss, "Switch statement expects value of type int or a protocol tag.", 3062);

        // check if we have alrady switched on a protocol of a similar name - if not add
        // it (after we have the appropriate case.
        if (eType.isProtocolType()) {
            ProtocolTypeDecl pt = (ProtocolTypeDecl) eType;
            if (protocolTagsSwitchedOn.containsKey(pt.name().getname()))
                Error.addError(ss.expr(), "Nested switch statements on the same protocol type is not allowed.", 3063);
        }

        ProtocolTypeDecl pt = null;
        for (SwitchGroup sg : ss.switchBlocks()) {
            // Only one protocol tag per case, so only one label per switch group. (no fall
            // through)
            if (eType.isProtocolType() && sg.labels().size() > 1)
                Error.addError(sg, "Fall-through cases in protocol switch statement not allowed.", 3064);
            for (SwitchLabel sl : sg.labels()) {
                // No default case in protocol switches
                if (sl.isDefault()) {
                    if (eType.isProtocolType())
                        Error.addError(sl.expr(), "Default case not allowed in protocol switch.", 3065);
                    else
                        continue;
                }
                ...
                if ((!(sl.expr() instanceof NameExpr)) && (!lType.isIntegralType() || lType.isLongType()))
                    Error.addError(sl, "Switch labels must be of type int or a protocol tag.", 3044);
                else if (sl.expr() instanceof NameExpr && !eType.isProtocolType())
                    Error.addError(sl.expr(), "Switch label must be of integer type.", 3066);
                else if ((!(sl.expr() instanceof NameExpr)) && eType.isProtocolType())
                    Error.addError(sl.expr(), "Switch label must be a protocol case name.", 3067);
                else if ((!(sl.expr() instanceof NameExpr)) && !sl.expr().isConstant())
                    Error.addError(sl, "Switch labels must be constants.", 3045);
                else {
                    if (sl.expr() instanceof NameExpr) {
                        pt = (ProtocolTypeDecl) eType;
                        ProtocolCase pc = findProtocolCase(pt, ((NameExpr) sl.expr()).name().getname());
                        if (pc == null)
                            Error.addError(sl, "Protocol tag '" + sl.expr() + "' not found in protocol '"
                                    + ((ProtocolTypeDecl) eType).name().getname() + "'.", 3060);
                        else
            ...
            if (eType.isProtocolType() && sg.statements().size() > 0)
                if (!(sg.statements().child(sg.statements().size() - 1) instanceof BreakStat))
                    Error.addError(sg, "Fall-through cases in protocol switch statement not allowed.", 3069);
            if (eType.isProtocolType())
            ...
            for (SwitchLabel sl : sg.labels()) {
                if (sl.isDefault()) {
                    if (ht.contains("default"))
                        Error.addError(sl, "Duplicate default label.", 3046);
                    else
                        ht.add("default");
                    continue;
                }
                ...
                if (ht.contains(strval))
                    Error.addError(sl, "Duplicate case label.", 3047);
                else {
                    ht.add(strval);
                }
            }
        ...
        if (eType.isProtocolType()) {
            // remove the protocl name from the hash table of protocols we are switching on
            // currently.
            protocolTagsSwitchedOn.remove(pt.name().getname());
        }
        return null;
    ...
    public Type visitSyncStat(SyncStat ss) {
        ...
        if (!t.isBarrierType())
            Error.addError(ss, "Non-barrier type in sync statement.", 3048);
        return null;
    }
    ...
    public Type visitTernary(Ternary te) {
        ...
        if (!eType.isBooleanType())
            Error.addError(te, "Non-boolean Expression found as test in ternary expression.", 3070);
        ...
        if (trueBranchType instanceof PrimitiveType && falseBranchType instanceof PrimitiveType) {
            ...
            // TODO: What about assignments of protocol and records wrt to their inheritance
            // and procedures?
        } else
    ...
    public Type visitTimeoutStat(TimeoutStat ts) {
        ...
        if (!eType.isTimerType())
            Error.error(ts, "Timer type required in timeout statement.", false, 3050);
        return null;
    }
