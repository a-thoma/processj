...
//                ProtocolTypeDecl ptd = (ProtocolTypeDecl) o;
    ...
    public Object visitLocalDecl(LocalDecl ld) {
         ...
         *   T x = a.read() + b.read() + ... + z.read();  // Multiple channel reads
         *   T x = read();                                // A Java method that returns a value
         *   T x = a + b;                                 // A binary expression
         *   T x = a = b ...;                             // A complex assignment statement */
...
//            type = PJRecord.class.getSimpleName();
    ...
    public Object visitChannelType(ChannelType ct) {
        ...
        /* Resolve parameterized type for channel, e.g. chan<T> where 'T'
    ...
    public Object visitChannelEndType(ChannelEndType ct) {
        ...
        /* Resolve parameterized type for channels, e.g. chan<T> where 'T'
         * is the type to be resolved */
        String type = getChannelType(ct.baseType());
    ...
    public Object visitRecordAccess(RecordAccess ra) {
        ...
        if (ra.record().type.isRecordType()) {
            ...
            if (d_protName2ProtTag.containsKey(String.format("%s.%s", protocName, d_currProtocolTag))) 
                protocName = d_protName2ProtTag.get(String.format("%s.%s", protocName, d_currProtocolTag));
            
            stAccessor.add("protocName", protocName);
            stAccessor.add("tag", d_currProtocolTag);
